...
- `README.md`: the generated workspace note.
...
It versions the workspace contract and local files, while `maestro install` initializes the workspace root as a Git repository when needed and materializes managed Git repositories under `repos/`.
...
If the repositories declare `bootstrap.strategy: auto`, Maestro detects the local toolchain from the materialized checkout. In the validated Symfony backend flow, both repositories resolved to Composer and `bootstrap` ran `composer install --no-interaction --prefer-dist`.

## 9. Validate the installed workspace

Run `doctor` once the repositories and projections are present:

```bash
maestro doctor --workspace .
```

If you want Maestro to install dependencies automatically, declare `spec.repositories[].bootstrap.strategy: auto` or `manual` in `maestro.yaml` and use `maestro bootstrap --workspace .` as the execution step. The workspace file does not run instantiation scripts by itself.
